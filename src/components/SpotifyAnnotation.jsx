...
const SpotifyAnnotation = ({ position, i, selected }) => {
	...
	const annotations = [
		{
			...
			arrow: (
				<svg
					viewBox="0 0 24 24"
					className="h-6 absolute top-5 right-4"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
				>
					<g
	...
	const annotation = annotations[i] ? annotations[i].selected : null;
						...
						{isModalOpen && (
							<div className=" p-1 rounded-none transition-all">
								<p className="text-white text-nowrap text-xs mt-2">
									<span>fatih</span>
									<span>{annotations[i] ? annotations[i].arrow : null}</span>
								</p>
							</div>
						)}
					</div>
...
export default SpotifyAnnotation;
